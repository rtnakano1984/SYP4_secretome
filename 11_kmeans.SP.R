...
logFC   <-   as.matrix(read.table(paste(processed_data, "logFC_mat.SP.txt", sep=""),   sep="\t", header=T, row.names=1, check.names=F, stringsAsFactors=F))
...
pdf(file=paste(fig, "dendrogram_hclust_of_k_mean_clusters_", k, ".SP.pdf", sep=""))
plot(hclust)
dev.off()
...
write.table(sorted_logFC, file=paste(processed_data, "sorted_logFC.kmeans.SP.txt", sep=""), row.names=T, col.names=NA, sep="\t", quote=F)
...
cluster <- cluster[idx,]
write.table(cluster, file=paste(stat, "k_means_", k, ".sorted_hclust.SP.txt", sep=""), sep="\t", quote=F, col.names=T, row.names=F)
