...
write.table(logFC, paste(processed_data, "logFC_mat.txt", sep=""), quote=F, sep="\t", row.names=T, col.names=NA)
...
out <- data.frame("#AIC/BIC Likelihodd test",
	...
	k)
write.table(out, file=paste(stat, "AIC_BIC_best_k.logFC.txt", sep=""), quote=F, row.names=F, col.names=F, sep="\n")
